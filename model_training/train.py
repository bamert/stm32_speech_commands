...
def get_param_grid(mode):
    ...
    if mode == "1d":
        ...
        for c, b, (m, h) in itertools.product(channels, blocks, input_res):
            yield {"n_channel": c, "n_blocks": b, "n_mels": m, "hop_length": h}
    else:
...
def run_experiment(
    ...
    onnx_path = checkpoint_callback.best_model_path.replace(
        ".ckpt", f"_{checkpoint_suffix}.onnx"
    )
...
def main(args):
    ...
    if args.grid:
        ...
        with open(results_file, "w") as f:
            f.write("ID | Accuracy | Hyperparameters\n")
            f.write("-" * 80 + "\n")
        ...
        for i, params in enumerate(grid):
            ...
            with open(results_file, "a") as f:
                res_line = f" {i} | {best_acc:.4f} | | {params}\n"
                f.write(res_line)

        print(f" Grid search complete! Results saved to {results_file}")

    else:
        best_acc, prof = run_experiment(
            {}, args.mode, sample_rate_hz, "", args.checkpoint
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Training script for audio classifier."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        choices=["1d", "2d"],
        default="1d",
        help="Choose pipeline to run",
    )
    parser.add_argument(
        "--grid", action="store_true", help="Flag to run the hyperparameter grid search"
    )
