...
class M5(nn.Module):
    ...
    """
    def __init__(self, n_input=1, n_output=35, stride=16, kernel_size=64, 
                 n_channel=32, n_blocks=4, pool_size=4):
        super().__init__()
        input_layer = []
        input_layer.append(nn.Conv1d(n_input, n_channel, kernel_size=kernel_size, stride=stride))
...
class Dilated1DNano(nn.Module):
    def __init__(self, n_output=36, base_channels=24):
        ...
        self.conv1 = nn.Conv1d(1, base_channels, kernel_size=24, stride=16, padding=12, bias=False)
        self.bn1 = nn.BatchNorm1d(base_channels)
        self.pool1 = nn.MaxPool1d(4, stride=4) 

        self.dw1 = nn.Conv1d(base_channels, base_channels, kernel_size=3, padding=1, groups=base_channels, bias=False)
        self.pw1 = nn.Conv1d(base_channels, 32, kernel_size=1, bias=False)
        self.bn1_dw = nn.BatchNorm1d(32)

        self.dw2 = nn.Conv1d(32, 32, kernel_size=3, padding=2, dilation=2, groups=32, bias=False)
        self.pw2 = nn.Conv1d(32, 36, kernel_size=1, bias=False)
        self.bn2_dw = nn.BatchNorm1d(36)
        
        self.dw3 = nn.Conv1d(36, 36, kernel_size=3, padding=4, dilation=4, groups=36, bias=False)
...
class Dilated1DMicro(nn.Module):
    def __init__(self, n_output=36, base_channels=24):
        ...
        self.conv1 = nn.Conv1d(1, base_channels, kernel_size=64, stride=32, padding=16, bias=False)
        self.bn1 = nn.BatchNorm1d(base_channels)
        ...
        self.dw2 = nn.Conv1d(base_channels, base_channels, kernel_size=3, padding=2, dilation=2, groups=base_channels, bias=False)
        self.pw2 = nn.Conv1d(base_channels, 32, kernel_size=1, bias=False)
        self.bn2_dw = nn.BatchNorm1d(32)
        
        self.dw3 = nn.Conv1d(32, 32, kernel_size=3, padding=4, dilation=4, groups=32, bias=False)
...
class Dilated1DBalanced(nn.Module):
    def __init__(self, n_output=36, base_channels=24):
        ...
        self.conv1 = nn.Conv1d(1, base_channels, kernel_size=32, stride=16, padding=32, bias=False)
        self.bn1 = nn.BatchNorm1d(base_channels)
        self.pool1 = nn.MaxPool1d(4, stride=4) 
        ...
        self.dw3 = nn.Conv1d(48, 48, kernel_size=3, padding=4, dilation=4, groups=48, bias=False)
...
class Dilated1D(nn.Module):
    def __init__(self, n_output=36, base_channels=32):
        ...
        self.conv1 = nn.Conv1d(1, base_channels, kernel_size=128, stride=16, padding=64, bias=False)
        self.bn1 = nn.BatchNorm1d(base_channels)
        self.pool1 = nn.MaxPool1d(4, stride=4) 
        ...
        self.dw2 = nn.Conv1d(base_channels, base_channels, kernel_size=3, padding=2, dilation=2, groups=base_channels, bias=False)
        self.pw2 = nn.Conv1d(base_channels, base_channels * 2, kernel_size=1, bias=False)
        self.bn2_dw = nn.BatchNorm1d(base_channels * 2)
        
        self.dw3 = nn.Conv1d(base_channels * 2, base_channels * 2, kernel_size=3, padding=4, dilation=4, groups=base_channels * 2, bias=False)
        self.pw3 = nn.Conv1d(base_channels * 2, base_channels * 2, kernel_size=1, bias=False)
...
class DSCNN(nn.Module):
    def __init__(self, n_input=1, n_output=36, n_channel=64, n_blocks=2):
        ...
        self.conv1 = nn.Conv2d(n_input, n_channel, kernel_size=(5, 4), stride=(2, 2), padding=(2, 1))
        self.bn1 = nn.BatchNorm2d(n_channel)
        
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        self.dw1 = nn.Conv2d(n_channel, n_channel, kernel_size=3, padding=1, groups=n_channel, bias=False)
        ...
        self.dw2 = nn.Conv2d(n_channel, n_channel, kernel_size=3, padding=1, groups=n_channel, bias=False)
